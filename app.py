...
def compute_explainability_score(
    ...
    monitoring_readiness = _clamp01(hits / 4.0)
    ...
    narrative = (
        ...
        f"Evidence trace is {evidence_trace:.2f}. "
        f"Reasoning transparency is {reasoning_transparency:.2f} "
        f"(dynamic threshold={thr_s} + activation explanations). "
        f"Control coverage is {control_coverage:.2f}. "
        f"Monitoring readiness is {monitoring_readiness:.2f}. "
...
def build_explainability_brief(
    ...
    if lang == "ja":
        ...
        no_evidence = "- まだ証拠が一致していません。"
        no_activation = "- 発火した詳細はありません。"
        interpretation = (
            "このツールは単なるコンプライアンス・チェックリストではない。"
            "組織内承認、監査対応、インシデント振り返り、経営説明に再利用できる"
            "再現可能な意思決定トレースを提供する。"
        )
    else:
        ...
        no_evidence = "- No evidence matched yet."
        no_activation = "- No activation details available."
        interpretation = (
            "This tool is not merely a compliance checklist. "
            "It produces an organizationally explainable decision trace "
            "that can be reused for internal approvals, audit readiness, "
            "incident retrospectives, and governance communication."
        )
...
def search_clauses(
    ...
    model = load_embedding_model(model_name)
    q_emb = embed_texts(model, model_name, [query], is_query=True)
    sims = (clause_emb @ q_emb.T).reshape(-1)
...
def compute_dynamic_threshold(sim_matrix: np.ndarray, floor: float = 0.78, ceil: float = 0.90) -> float:
    ...
    thr = p + 0.15 * spread
    return float(max(floor, min(ceil, thr)))
...
def risk_mapping_embedding(
    ...
    sim_matrix = cosine_sim_matrix(c_emb, r_emb)
    ...
    return sorted(list(activated)), activation_details, sim_matrix, risk_ids
...
def recommend_controls_for_risks(activated_risks: List[str]) -> List[str]:
    ...
    return [cid for cid, s in ranked if s > 0][:6]


# =========================
# Step 3 — Risk Score + Coverage
# =========================
...
def control_coverage_gap(
    ...
    for cid in recommended_controls or []:
        for r in CONTROL_CATALOG.get(cid, {}).get("covers", []):
            covered.add(r)
        ...
        for ctrl in recommended_controls or []:
            if r in CONTROL_CATALOG.get(ctrl, {}).get("covers", []):
                ...
                if rn in G and cn in G:
                    G.add_edge(rn, cn, weight=rw, kind="risk_control")
...
def build_google_news_rss(jurisdiction: str, extra_terms: str, recency_days: int) -> str:
    ...
    q = (base_terms + f" when:{int(recency_days)}d").replace(" ", "+")
...
def build_next_steps(
    ...
    if lang == "ja":
        ...
        risk_action_map = {
            ...
            "security_risk": [
                ...
                "運用上の攻撃面ごとに監視ポイントを決める。",
            ],
            "hallucination_risk": [
                "高リスク出力について human review / fallback を導入する。",
                "評価データセットを用意し、事前評価と継続監視の仕組みを作る。",
            ],
            "transparency_risk": [
                "利用者向けにAI利用、限界、人間への引き継ぎ条件を明示する。",
                "どの証拠からどの判断に至ったかを記録する説明ログを整備する。",
            ],
            "accountability_risk": [
                "承認者、運用責任者、事故対応責任者を明確化する。",
                "変更管理・例外承認・定期レビューのルールを作る。",
            ],
            "bias_risk": [
            ...
            "incident_risk": [
                "triage → rollback/fallback → 報告 → 振り返りの手順を整備する。",
                "異常検知の閾値と停止判断者を事前に決める。",
            ],
        }
        ...
        control_action_map = {
            ...
            "transparency_notice": "利用者向け説明と内部記録テンプレートを優先して整備する。",
        ...
        intro = []
        if risk_level == "HIGH":
            intro.append("現在のリスク水準は高いため、高影響リスクへの即応策を優先する。")
        elif risk_level == "MEDIUM":
            intro.append("現在のリスク水準は中程度であり、運用ルールと監視体制の整備を優先する。")
        else:
            intro.append("現在のリスク水準は比較的低いが、最小限の統制を先に整える。")

        if evidence_count < 5:
            intro.append("根拠条項が少ないため、証拠データセットの拡充が必要である。")
        ...
        if missing_controls:
            ...
            for ctrl in missing_controls:
                lines.append(f"- {control_action_map.get(ctrl, f'`{ctrl}` を優先的に整備する。')}")

        if recommended_controls:
            lines.append("")
            lines.append("**優先して運用に載せる統制**")
            for ctrl in recommended_controls[:4]:
                lines.append(f"- {control_action_map.get(ctrl, f'`{ctrl}` を実装対象に含める。')}")

        if coverage_ratio < 0.50:
            lines.append("")
            lines.append("- 現在の統制カバー率が低いため、不足コントロールの補完を最優先にする。")
        elif coverage_ratio > 0.95:
            lines.append("")
            lines.append("- 統制カバー率は高いが、過剰検出の可能性もあるため、しきい値や定義を見直す。")
    ...
    risk_action_map = {
        ...
        "security_risk": [
            ...
            "Define monitoring points for the main attack surfaces.",
        ],
        "hallucination_risk": [
            "Introduce human review / fallback for high-impact outputs.",
        ...
        "accountability_risk": [
            "Clarify who approves, who operates, and who owns incident response.",
            "Create change control, exception approval, and recurring review rules.",
        ],
        "bias_risk": [
            "Audit dataset and performance differences across groups, and operationalize fairness evaluation.",
            "Keep human review in the loop for high-impact use cases.",
        ],
        "incident_risk": [
            "Create a triage → rollback/fallback → reporting → postmortem workflow for AI incidents.",
            "Define escalation thresholds and stop/go decision owners in advance.",
        ],
    }
    ...
    if missing_controls:
        ...
        for ctrl in missing_controls:
            lines.append(f"- {control_action_map.get(ctrl, f'Prioritize `{ctrl}`.')}")

    if recommended_controls:
        lines.append("")
        lines.append("**Controls to operationalize next**")
        for ctrl in recommended_controls[:4]:
            lines.append(f"- {control_action_map.get(ctrl, f'Operationalize `{ctrl}`.')}")
...
with st.sidebar:
    ...
    jurisdiction = st.selectbox(
        ...
        ["JP", "AU", "EU", "US", "UK", "CA"],
        index=0,
        help="Choose where governance evidence should come from. Add your own corpus under law_corpus/<JUR>/clauses.json or clauses.jsonl.",
    ...
    benchmark_mode = st.checkbox(
        "Use benchmark case",
        value=False,
        key="sidebar_benchmark_mode_checkbox",
    )
    ...
    use_auto_threshold = st.checkbox(
        t(lang, "use_dynamic_threshold"),
        value=True,
        key="sidebar_auto_threshold_checkbox",
    )
    manual_threshold = st.slider(
        t(lang, "manual_threshold"),
        0.70, 0.95, 0.83, 0.005,
        key="sidebar_manual_threshold_slider",
    )
    ...
    node_alpha = st.slider(
        t(lang, "node_alpha"),
        0.50, 1.00, 0.92,
        key="sidebar_node_alpha_slider",
    )
    edge_alpha = st.slider(
        t(lang, "edge_alpha"),
        0.10, 1.00, 0.55,
        key="sidebar_edge_alpha_slider",
    )
    edge_width_scale = st.slider(
        t(lang, "edge_width"),
        0.5, 6.0, 3.0,
        key="sidebar_edge_width_slider",
    )
    ...
    enable_rss = st.checkbox(
        t(lang, "fetch_rss"),
        value=True,
        key="sidebar_enable_rss_checkbox",
    )
    recency_days = st.slider(
        t(lang, "recency_days"),
        7, 60, 30,
        key="sidebar_recency_days_slider",
    )
    ...
    force_rebuild = st.checkbox(
        ...
        value=False,
        help="Use if you updated clauses.json or clauses.jsonl.",
        key="sidebar_force_rebuild_checkbox",
    )
...
with left:
    ...
    if not clauses:
        st.error(
            ...
            f"- {clauses_jsonl} (preferred)\n"
            f"- {clauses_json} (fallback)\n"
        )
        st.stop()
    ...
    with st.expander("Show clause schema expectations / tips", expanded=False):
        st.markdown(
            """
- `clauses.jsonl` is preferred.  
- `clauses.json` is also supported.  
- Each clause should include: `clause_id`, `title`, `text`, `url`.  
"""
        )
    ...
    thr = None if use_auto_threshold else float(manual_threshold)
